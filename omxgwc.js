...
jQuery(document).ready(function() { console.log('JS Loaded - 15'); });


// Some stuff to do when document.ready:
jQuery(document).ready(function() {
	// Add the span elements needed to add the styled name and number
	jQuery('#wccf_product_field_name_style_container li > label').after('<span class="styled ridernumber" style="margin-left:10px"></span>').after('<span class="styled ridername" style="margin-left:10px"></span>');
...
// Add the font CSS style for each syled name and number
jQuery(document).ready(function() {
...
// Dynamically change the color of rider number
jQuery('#wccf_product_field_number_color').change(function(e) {
	var chosencolor = jQuery(this).val();
	jQuery('#wccf_product_field_rider_number').css('color', chosencolor );	
});
// Dynamically change the background of number plate
jQuery('#wccf_product_field_number_plate_color').change(function(e) {
	var chosenbkcolor = jQuery(this).val();
	jQuery('#wccf_product_field_rider_number').css('background-color', chosenbkcolor );	
});
