...
function moomx_rearrange_woocomemrce_features() {
	...
	add_action( 'woocommerce_proceed_to_checkout', 'woocommerce_button_proceed_to_checkout', 0 );
}

// Add Select options buttons to all products
add_action( 'astra_woo_shop_summary_wrap_bottom', 'moomx_select_options_button', 10 );
function moomx_select_options_button() {
	global $product;
	echo'<a href="' . $product->get_permalink() . '" data-quantity="1" class="button product_type_variable add_to_cart_button omx_select_options_button" data-product_id="' . $product->get_id() . '" data-product_sku="' . $product->get_sku() . '" aria-label="Select options for “' . $product->post->post_title . '”" rel="nofollow">Select options</a>';
}
...
function moomx_change_addtocart_notice($products) {
	$addtocart_notice = 'Well done! <br>Just one more step to get your order on its way.<br><a href="/shop/" class="continue-shopping_cart-button-top button alt wc-forward">Or continue shopping.</a>';
...
if ( ! function_exists( 'woocommerce_button_proceed_to_checkout' ) ) {
	function woocommerce_button_proceed_to_checkout() {
		?>
		<a href="<?php echo wc_get_checkout_url() ?>" class="checkout-button button alt wc-forward"><?php _e( 'Checkout' ); ?></a>
		<p id="omx-payment-request-button-separator">— OR —</p>
		<?php
	}
}

// Adding the security icons in the Cart and Checkout pages
add_action( 'woocommerce_proceed_to_checkout', 'moomx_security_icons', 100);
add_action( 'woocommerce_review_order_after_submit', 'moomx_security_icons', 1);
function moomx_security_icons() {
	echo '<div class="security-icons-container"><img class="sharp-image" src="' . plugin_dir_url( __FILE__ ).'images/SecureIconsCheckout02.png"></div>';
}

// Add Agree to Terms in Cart
add_action( 'woocommerce_proceed_to_checkout', 'moomx_cart_privacy_policy_checkbox', 110 );
function moomx_cart_privacy_policy_checkbox() {
	echo '<div class="cart-terms-agreement">By placing the order you agree to our <a href="/terms-conditions/" target="_blank">Terms and Conditions</a>, <a href="/privacy-policy/" target="_blank">Privacy</a> and <a href="/returns-exchanges/" target="_blank">Return</a> policies</div>';
}
